...
class OnlineAdapter:
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        seed: int,
        cfgs: Config,
        make_env_fn: Optional[Callable] = None  # NEW
    ) -> None:
        """Initialize an instance of :class:`OnlineAdapter`."""
        if "make_env_fn" in cfgs:
            make_env_fn = cfgs.make_env_fn
        ...
        if "step_callback" in cfgs:
            self._step_callback = cfgs.step_callback
        else:
            def dummy_step_callback():
                pass
        ...
        if make_env_fn is not None:
            ...
            self._env: CMDP = make(env_id, class_name=None, num_envs=num_envs, device=self._device, seed=seed)
            self._eval_env: CMDP = make(env_id, class_name=None, num_envs=1, device=self._device, seed=seed)
