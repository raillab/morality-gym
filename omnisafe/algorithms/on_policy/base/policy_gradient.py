...
class PolicyGradient(BaseAlgo):
    ...
    def learn(self) -> tuple[float, float, float]:
        ...
        # Run before training for curr_step = 0 eval
        # Callback function that is called every epoch and passes PolicyGradient obj as param
        # if self._cfgs.train_cfgs.eval_freq > 0:
        #     actor = self._actor_critic.actor
        #     eval_env = self._env._eval_env
        #     self._cfgs.train_cfgs.eval_callback(actor, eval_env, self._logger, 0, 0)
        #
        ...
        for epoch in range(self._cfgs.train_cfgs.epochs):
            ...
            # Callback function that is called every epoch and passes PolicyGradient obj as param
            # if self._cfgs.train_cfgs.eval_freq > 0 and (epoch % self._cfgs.train_cfgs.eval_freq == 0 or
            #                                             epoch == self._cfgs.train_cfgs.epochs - 1):  # Eval on final epoch
            #     actor = self._actor_critic.actor
            #     print("\n\n\n")
            #     print("######################################")
            #     print("Eval on epoch: ", epoch + 1, " ...")
            #     print("######################################")
            #     print("\n\n\n")
            #     eval_env = self._env._eval_env
            #     curr_step = (epoch + 1) * self._steps_per_epoch
            #     self._cfgs.train_cfgs.eval_callback(actor, eval_env, self._logger, epoch+1, curr_step)
