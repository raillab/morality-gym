...
from morality_gym.morality_chain.norm import Norm, UtilityNorm, DeonticModalityEnum
...
class MoralityChain:
    def __init__(
        ...
        # if schema is None and norms is None:
        #     raise ValueError("Either schema or norms must be provided")
        #
        # if schema is not None:
        #     self._norms = self._build_from_schema(schema)
...
def main():
    ...
    env, mt = env_mt_make(
        ...
        env_kwargs={"seed": 42, "obs_type": dict},
    ...
    print(f"Morality metric: {round(morality_metric, 3)}")
    print(f"Morality functions:")
    morality_functions = {name: round(val, 3) for name, val in morality_functions.items()}
    pprint.pprint(morality_functions)
    print(f"Avg. return: {avg_return}")
    print("Info:")
    pprint.pprint(info)
    # print(random_policy(None))
