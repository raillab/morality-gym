...
from morality_gym._morality_chain.norm import UtilityNorm, DeonticModalityEnum, EventNorm
...
def _make_utilitarian(
    ...
    if morality_tree_name == "UtilityHarm":
        ...
        norms = [norm]
        # _morality_chain = MoralityChain(norms=[norm], **morality_chain_kwargs)
        # return _morality_chain
    ...
    # OrderedUtilityHarm #
        ...
        # _morality_chain = MoralityChain(norms=[human_norm, animal_norm, robot_norm], **morality_chain_kwargs)
        # return _morality_chain
    ...
    # OrderedOutcomeHarm #
        ...
        norms = [human_norm, animal_norm, agent_norm, robot_norm]
        # _morality_chain = MoralityChain(norms=[human_norm, animal_norm, agent_norm, robot_norm], **morality_chain_kwargs)
        # return _morality_chain
...
def _make_dual_process(
    ...
    if morality_tree_name == "Simple":
        ...
        # _morality_chain = MoralityChain(norms=[personal_harm_norm, n_harm_norm], **morality_chain_kwargs)
        # return _morality_chain
        ...
        # _morality_chain = MoralityChain(norms=norms, **morality_chain_kwargs)
        # return _morality_chain
...
def make(
    ...
    if morality_chain_id in MORALITY_CHAIN_ID_ALIASES:
        morality_chain_id = MORALITY_CHAIN_ID_ALIASES[morality_chain_id]
    elif morality_chain_id in MORALITY_CHAIN_ID_ALIASES.values():
        pass
    else:
        raise ValueError(f"Unsupported morality chain id: {morality_chain_id}. Supported morality chains = {list(MORALITY_CHAIN_ID_ALIASES.keys())}")

    arr = morality_chain_id.split("-")
    env_type = arr[0]
    env_subtype = arr[1]
    mt_group = arr[2]
    mt_name = arr[3]
    ...
    if env_type == "Trolley":
        if env_subtype == "Common":
            if mt_group == "Utilitarian":
                morality_tree = _make_utilitarian(mt_name, utility_bounds, **morality_chain_kwargs)
            elif mt_group == "DualProcess":
                morality_tree = _make_dual_process(mt_name, utility_bounds, **morality_chain_kwargs)
            else:
